    ...
    try:
        ...
        if data.get("list"):
            ...
            for chunk in info.get("vod_play_url", "").split("$$$"):
                if ".m3u8" in chunk: 
                    play_url = chunk; found_m3u8 = True; break
            if not found_m3u8: play_url = info.get("vod_play_url", "").split("$$$")[0]
...
def heartbeat():
    ...
    # 在后台日志中标记为 [Active]，每收到一次代表用户停留了30秒
    log_traffic(f'[Active] {page}')
    return "ok"
...
def admin_stats():
    try:
        with sqlite3.connect(DB_FILE) as conn:
            ...
            # 计算时长：统计有多少个 [Active] 信号，每个信号代表30秒
            count_active = sum(1 for (a,) in all_actions if '[Active]' in a)
            total_minutes = count_active * 0.5 # 30秒 = 0.5分钟
            ...
            <div style="display:flex;gap:10px;flex-wrap:wrap">
                ...
                <div class="card" style="flex:1"><b>播放次数</b><br><span style="font-size:24px;color:#28a745">{count_play}</span></div>
                <div class="card" style="flex:1"><b>估算总时长(分)</b><br><span style="font-size:24px;color:#dc3545">{int(total_minutes)}</span></div>
            </div>
            ...
            <div class="card"><h3>最近活动</h3><ul style="padding-left:20px;font-size:13px;color:#555">
