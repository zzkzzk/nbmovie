...
# 【管理员设置】
# 这里的空列表 [] 代表记录所有人的访问。
# 如果你不想记录自己的IP，可以填入 ["你的公网IP"]，但为了保险起见，建议先留空。
ADMIN_IP_FILTER = [] 
...
def play_handler():
    ...
    log_traffic(f'播放: ID-{vod_id} 集-{ep_index}') # 探针
